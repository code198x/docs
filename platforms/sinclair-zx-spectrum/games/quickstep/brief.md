...
**Status:** The user chose the crossing-and-timing direction after Drift. This bounded prototype tests the experience before a finished scope or lesson count is agreed.
...
Cross three lanes of predictable moving vehicles, rest between them, and reach the marked exit. Observe a gap, choose a step, then decide whether to continue or wait. One compact board, fixed repeating patterns, immediate failure and retry. No countdown or life budget. Success ends the crossing and offers another attempt.
...
A 13-column, seven-row logical board uses 16×16-pixel cells. Odd rows are hazard lanes; even rows are resting strips. Each lane has two vehicles occupying two cells apiece, separated by predictable gaps. Lane positions wrap; the middle lane moves in the opposite direction and on a different step period.
