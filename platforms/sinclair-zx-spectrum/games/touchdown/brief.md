...
**Route:** Meet BASIC → Bright Spark → Touchdown. Use the game identity rather than an inherited numbered slot.
...
Next: inspect the existing lessons and samples, build and play the bounded prototype, record configuration-specific findings, then derive the lesson/checkpoint breakdown. Preserve published URLs when replacement lessons are eventually published.
