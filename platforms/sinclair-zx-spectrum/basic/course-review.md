...
| Boards and deduction | Sonar, Crates | Reading a grid, combining clues and changing board state safely |
...
These are navigation groups, not prerequisites or separate courses. All current game numbers, URLs and relative order remain unchanged. The six earlier games remain separately available, and the BASIC reference remains optional. Future games need not be appended to the last group: a revised Locksmith could join Boards and deduction.
...
| Locksmith — `locksmith/unit-06/locksmith.bas` | Combine exact-position and wrong-position clues; counting repeated values without double-counting | Strong next candidate for Boards and deduction. Retain one four-digit code, six symbols and a visible guess history as the basis for a brief; decide the guess budget through play. Explain duplicates explicitly. It develops Sonar's deduction through a different clue algorithm. |
...
- **Three in a Row:** the strategy lesson claims a careful player cannot beat the opponent. The source tries win, block, centre, corners and edges; there is no explicit fork handling or search. Treat invincibility as unverified until explored over all legal player replies. The explanation also refers to an absent line 330. Neither claim should survive automatically into a rewrite.
...
1. Agree a bounded Locksmith brief, then prototype and judge its deduction and presentation. Do not begin lesson rewriting from the inherited brief.
2. Review Three in a Row as an opponent-design game, then The Caverns and Yearfall as contrasting forms of persistent state. These four have sources to inspect and distinct player decisions.
