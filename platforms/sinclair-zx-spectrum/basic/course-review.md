...
These are navigation groups, not prerequisites or separate courses. All current game numbers, URLs and relative order remain unchanged. Dice Roller is available as the opening probability experiment in Maths for Games, and the BASIC reference remains optional. Revised games can join the group that fits their activity; Locksmith and Three in a Row now belong to Boards and deduction.
...
| Night Patrol | Retained specification: patrol routes and wall-blocked sightlines. Catalogue: multiple targets and limited interceptors | The user selected stealth and larger maps. The current brief defines a 30 × 18 archive trial with one guard, wall-blocked visible sight and a retrieve-and-return objective. The corner-scanning native game is approved; eight checkpoints pass 96 native checks, and the approved overview plus ten lessons are published in Hidden worlds. |
...
3. The user selected Cipher and subsequently authorised its compact lesson sequence; the six approved lessons with five verified checkpoints are published in Boards and deduction. Dice Roller’s live prototype and compact five-stage teaching sequence are approved; its five lessons are published in Maths for Games as a probability experiment, with the old URLs preserved through redirects. No retained published BASIC project remains unchanged from this review.
4. The user explicitly accepts not developing every proposal. Night Patrol is now selected as stealth, with a larger-map native trial. Assess the other proposal families individually. New groups should follow accepted experiences; do not publish empty groups or a promised finale sequence.
