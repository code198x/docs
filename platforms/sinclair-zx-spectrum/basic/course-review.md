...
3. The user selected Cipher and subsequently authorised its compact lesson sequence; the six approved lessons with five verified checkpoints are published in Boards and deduction. Dice Roller remains the one unchanged published project. The user selected it for a bounded native probability experiment; the live prototype is approved, and its compact five-stage teaching sequence and runnable checkpoints are prepared. The replacement overview and five lessons are authored locally for review, with Dice Roller placed in Optional experiments.
