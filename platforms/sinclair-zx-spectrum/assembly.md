...
**Track:** Z80 Assembly
**Shape:** A tight **rendering ladder** of ~10 game modules after the Primer — a `teaching` Primer (*Meet the Machine*), *Gloaming*, *Shadowkeep*, then Lamplight → Ricochet → Deepdelve → Nibble Run → Skyhawk → Greypeak → Starfall → Hard Rain → Wyrmhold → the *Spectral Legacy* capstone. Each game's headline is one rendering rung; later games inherit the rungs below. Counts deliberately flexible. Modules — not games — are the unit of sequencing ([decisions/modules-not-games.md](../../decisions/modules-not-games.md)). The catalogue (`website/src/content/modules/sinclair-zx-spectrum/assembly.yaml`) is authoritative.
...
**Status:** Active. v0.5 (2026-06-10) — reconciled to the tight rendering ladder. The old 44-game lattice + V2–6 branches and the "Revised lineup" table further down are **retired** (see [decisions/spectrum-assembly-gentle-ramp.md](../../decisions/spectrum-assembly-gentle-ramp.md) § Volume 1); prose below this header still describing Underlight/Whitewinter/Embergate/Lantern Path/Last Banner or V2–6 is historical only.
