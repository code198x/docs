...
|------|-------|------------------------|---------------|
| 1 | Arrival | Title screen: "THE CURSED MANOR" in large PRINT AT text with colour. Atmospheric BEEP. "You arrive at Thornbury Manor..." Opening narration scrolls with cyan INK on black PAPER. Press any key to begin. | PRINT AT for title layout, INK/PAPER/BORDER, BEEP, CLS |
...
| 8 | The Cellar and Grounds | Add cellar rooms (accessed from kitchen stairs) and grounds (accessed from garden door). 30+ rooms now. Some cellar rooms print "It is dark here" with dim INK. The grounds use different atmospheric text — moonlight, cold air, gravel underfoot. | Conditional descriptions per room, atmospheric writing |
...
| 46 | The Ritual | In the tower study with amulet, lit candle, and name spoken: the curse breaks. Lady Ashworth unfreezes — the last guest. A dramatic sequence: the air shimmers, frost cracks, warmth returns, all remaining guests gasp awake. Green INK. Ascending BEEP scale. | Multi-condition check, victory sequence, dramatic climax |
...
| 61 | Optimisation | Move frequently-called GO SUBs earlier in the listing (room display, parser, NPC update). Use short variable names everywhere. Avoid string concatenation in loops. Test that the NPC turn does not cause a noticeable pause between commands. | BASIC interpreter performance, GO SUB placement, speed testing |
...
**Milestone:** A polished, complete graphical text adventure with autonomous NPCs. 35-40 rooms, 10 location illustrations drawn with PLOT and DRAW, 8 characters who move and act on their own, a multi-step puzzle chain, atmospheric colour and sound, and a world that breathes. Under 500 lines of Sinclair BASIC. The learner who typed `10 PRINT "Hello"` in Game 1 has built The Hobbit.
