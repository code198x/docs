; Ink War - Unit 11: Detecting Game End
; Detect when all 64 cells have been claimed
;
; Learning objectives:
; - Track game state (playing vs ended)
; - Detect when the board is full
; - Stop accepting moves when game is over
; - Simple state machine basics
...
game_over_loop:
        ; Game is over - just loop (next unit adds winner display)
        jr main_loop
...
; Returns: sets game_state to STATE_GAME_OVER if board is full
; -----------------------------------------------------------------------------
check_game_end:
        ; Count empty cells
        ld hl, board_state
