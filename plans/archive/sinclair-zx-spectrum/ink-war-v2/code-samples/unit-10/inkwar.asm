; Ink War - Unit 10: Score Display
; Display player scores at the top of the screen
;
; Learning objectives:
; - Count cells owned by each player
; - Convert numbers to printable characters
; - Write characters directly to screen memory
...
; Uses scratch variables to avoid stack complexity
; -----------------------------------------------------------------------------
print_two_digit:
        ; Save position
...
print_char:
        ...
        ; Address = $4000 + (row/8)*$800 + (row%8)*$20 + column
        ; But the tricky part: each character row spans 8 pixel lines
        ; and pixel lines are $100 apart

        ; For the FIRST pixel line of character row B:
        ...
        ; Within a third, successive pixel lines are $100 apart
        ld b, 8
print_char_loop:
        ld a, (de)
        ld (hl), a
