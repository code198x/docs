; ============================================================================
; SID Symphony - Unit 16: Visual Polish
; ============================================================================
; Adding visual polish and intro sequence:
; - Animated title screen with colour cycling
; - "Press SPACE to start" prompt
; - Smoother visual feedback effects
; - Pulsing hit zones to draw player attention
; ============================================================================
...
start:
                jsr setup_charset
                jsr title_screen        ; Show title first
                jsr init
...
+               ; Draw subtitle (25 chars, center at col 7)
                ldx #0
-               lda title_sub,x
                beq +
                sta SCREEN + 10*40 + 7,x
                lda #COL_GREY
                sta COLOUR + 10*40 + 7,x
                inx
                bne -

+               ; Draw press space prompt (19 chars, center at col 10)
                ldx #0
-               lda press_space,x
                beq +
...
+               ; Draw credits (19 chars, center at col 10)
                ldx #0
-               lda credits_text,x
                beq +
                sta SCREEN + 22*40 + 10,x
                lda #COL_DARK_GREY
                sta COLOUR + 22*40 + 10,x
                inx
                bne -
...
.check_space:
                ...
                lda KEYBOARD_ROW
                and #%00010000
                bne .title_loop         ; Not pressed, keep animating

                ; Space pressed - brief pause then start
                ldx #30
-               jsr wait_raster
                dex
                bne -
...
big_title:      !scr "sid symphony"
                !byte 0
title_sub:      !scr "a rhythm game for the c64"
                !byte 0
press_space:    !scr "press space to play"
                !byte 0
credits_text:   !scr "code like it's 198x"
                !byte 0
