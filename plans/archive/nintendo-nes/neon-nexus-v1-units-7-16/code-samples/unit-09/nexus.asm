...
; Game constants
NUM_ENEMIES = 4
HITBOX_SIZE = 7             ; Collision distance (pixels)
...
; CHECK COLLISION
;==============================================================
; Test if player overlaps any enemy using bounding box detection.

.proc check_collision
    ldx #0                  ; Enemy index

@loop:
    ; Calculate X distance: |player_x - enemy_x|
    lda player_x
    sec
    sbc enemy_x, x
    bcs @x_positive         ; Branch if result positive
    ; Negative - negate to get absolute value
    eor #$FF
    clc
    adc #1
@x_positive:
    cmp #HITBOX_SIZE        ; Compare to hitbox
    bcs @next_enemy         ; Too far in X, skip

    ; Calculate Y distance: |player_y - enemy_y|
    lda player_y
    sec
    sbc enemy_y, x
    bcs @y_positive
...
@y_positive:
    cmp #HITBOX_SIZE
    bcs @next_enemy         ; Too far in Y, skip
