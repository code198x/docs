...
3. **Content audit follow-ups.** H4, M4, M5 and L3–L8 are in review as website PRs #519–#523, with the owner's answers applied: the 66 placeholder module pages are removed (#523, redirects kept), the assembly pages' stage numbers are dropped, the Craft planning notes are folded into `specifications/`, and `check-vault-dates.mjs` no longer hard-codes a local path. L6's unverified manufacturer years render nowhere, so they need no change. One question remains: the Spectrum lessons all build with Asm198x, but in two dialects (`pasmonext --sna` and `pasmo --cpu z80 --tapbas`). H2, H3, H5, H7, H8 and L1 are fixed.
