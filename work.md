...
Batch 8 starts with entries earlier reviewers found wrong or unfit: `games/monty-on-the-run` (calls Hubbard's in-game music a loading theme; *ZZAP!64* 6 says it plays during the action), `magazines/the-games-machine-italy` (private-library wording; its end date rests on which issues were held), `companies/edizioni-hobby` (an unreadable registration number) and `people/julian-rignall` (unchecked *Mean Machines* and IGN claims), then the next most-linked: `communities/demo-scene`, `people/jay-miner`, `people/chuck-peddle`, `companies/ricoh`, `techniques/open-borders` and `techniques/fli`.
