...
3. **Content audit follow-ups.** H4, M4, M5 and L3–L8 are fixed (website #519–#523, merged 2026-09-25), so every audit finding outside the owner decisions is closed. One question remains: the Spectrum lessons all build with Asm198x, but in two dialects (`pasmonext --sna` and `pasmo --cpu z80 --tapbas`).
