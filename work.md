...
1. **The Vault.** The [September 2026 content audit](audit/2026-09-content-audit/REPORT.md) found no Vault entry reviewed and a sample of checked claims wrong one time in six (H1). The owner has made the Vault the next priority and chose the review process in [the Vault specification](specifications/vault.md#reviewing-existing-entries): batches of about ten, one pull request each, owner approval as sign-off, and unreviewed entries kept out of search engines. Batches 1 to 55 have merged (batch 55 as website PR #515); none are open. The pipeline is paused at the owner's request; batch 56 is queued with a *Rocky's Boots* follow-up. The owner approves a batch on its PR; `gh stack merge <pr> --rebase` merges it with everything below it. Each PR lists the entries it queues for the next batch. [Candidates](vault-candidates.md) lists missing entries. See [Vault editorial work](#vault-editorial-work).
2. **Browser-player integration design.** The design is written as website PR #525 (`docs/superpowers/specs/2026-09-25-browser-player-integration-design.md`) and awaits the owner's review before an implementation plan. System pages show our own capture as a poster that becomes the player; lessons get a run strip that opens a docked panel. It supersedes the overlapping parts of the [House UI preview design](https://github.com/code198x/website/blob/main/docs/superpowers/specs/2026-09-21-house-ui-preview-design.md). A panel inspector (registers, memory, labels) is a separate second project that needs Emu198x player and Debug198x work. The VIC-20 poster waits on emu198x/emu198x#1536.
