...
The board and manual cue stages of the [agreed Bright Spark brief](platforms/sinclair-zx-spectrum/games/bright-spark/brief.md) are implemented and checked; see the [prototype record](platforms/sinclair-zx-spectrum/games/bright-spark/prototype.md). The next bounded task is to settle automatic playback timing under key interference, then develop the opening replacement lessons. It retains the four-panel memory game, separates sequence representation from playback and input, and specifies score, replay, input hand-off and a finite ending. Seven teaching groups and a 16-round challenge are agreed starting defaults, not fixed curriculum constraints. Existing Bright Spark lessons remain published while the replacement is developed.
