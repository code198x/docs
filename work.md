...
Use the [Meet BASIC lesson and source mapping](platforms/sinclair-zx-spectrum/basic/meet-basic-plan.md) for implementation. A1–A4 are approved with verified target-side checkpoints; native A1–A3 acceptance is user-reported. A4 is approved with two verified checkpoints, completing Story Builder. The four-unit opening and Emu198x v0.22.0 are published. Begin Lucky Number with a known secret and useful clues. Reconcile Bright Spark and Touchdown's briefs with that opening. Keep subsequent BASIC and assembly choices as proposals for joint review.
