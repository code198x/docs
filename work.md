...
3. **Content audit follow-ups.** H4, M4, M5 and L3–L8 are in review as website PRs #519–#522 (CI green). Each PR lists its open questions for the owner: whether to remove the 66 placeholder module pages, whether to drop the "04 · Machine track" stage numbers, where the `UNIT-BREAKDOWN.md` notes belong, which Spectrum build command is canonical, and whether `check-vault-dates.mjs` should stop hard-coding a local path. L6's unverified manufacturer years render nowhere, so they need no change. H2, H3, H5, H7, H8 and L1 are fixed.
