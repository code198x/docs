...
Batch 9 starts with entries earlier reviewers found wrong or carrying private-library wording: `companies/xenia-edizioni`, `magazines/the-games-machine`, `culture/magazines-across-borders` and `magazines/club-nintendo` (all repeat the Italian *Games Machine*'s false May 1999 end), `companies/gremlin-graphics` (a value-for-money mark given as the overall score; the *Monty Mole* credit), `magazines/mean-machines` ("held here" wording) and `systems/atari-8-bit` (says Miner co-founded Hi-Toro). Fill the rest by re-ranking.
