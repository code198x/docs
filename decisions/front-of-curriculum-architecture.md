...
**Numbers & Bits and From Source to Silicon are the assembly arm.** They live in the Foundations section because they are cross-platform — shared by four assembly tracks, so taught once rather than four times — but they are not universal. Numbers & Bits is the number and bit literacy assembly leans on constantly. From Source to Silicon is its payoff: what an interpreter, a compiler and an assembler each are, and why *when* the translating happens decides whether an enemy chases you sixty times a second or stutters.
