...
| 2026-07-01 | **Launch on-ramp games to be brought to the endpoint-first bar before October** — the five shipped, forward-authored games (Gloaming, Shadowkeep, Starfield, Dash, Flock) get a winnability-gate + DoD pass as *validate-and-repair* (pivot to re-author only where a game genuinely can't be repaired). Guardrails: never regress the launch (shipped game stays `complete` until its rebuild provably passes), one game in-flight at a time, prove the pipeline on Gloaming first. Amends [endpoint-first-authoring.md](endpoint-first-authoring.md) (2026-07-01) to apply retroactively. |
