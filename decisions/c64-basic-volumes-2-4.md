...
> names settled in the 2026-06-10 naming pass (per
> [inspired-by-not-clones-naming.md](inspired-by-not-clones-naming.md)). The catalogue is authoritative
...
characters, H4) · Sidestep (hardware sprites, H5 — the headline) · Nibbler (state machines) · Bouncer
...
H8) · Salvage Run (many sprites + readout) · Dungeon (procedural data + inventory) · Trackside
(pseudo-3D) · Showtime (mini-game collection).

**Volume 4 — Systems and Saves** (persistence, scrolling, the capstone):
Glide Pt 1/2 (smooth scrolling, H9 → multi-screen world) · Strongroom Pt 1/2 (persistence → full
state save) · Clash (two-sprite combat) · Conductor (SID-synced gameplay) · Homestead (save-backed
sandbox) · Masterwork (capstone — integrates everything).

## Key choices

- **A "Meet the VIC-II" teaching module opens the hardware run** (before Glyph/Sidestep). The shipped
...
- **Custom characters (H4) before hardware sprites (H5).** A redefined-char *background* (Glyph)
  exists before the first sprite (Sidestep) moves over it.
- **Hardware sprites (H5) land as early as the language allows** (Sidestep, Volume 2). By then the
...
- **Names** — settled in the 2026-06-10 naming pass (Sidestep, Salvage Run, Homestead, Glide,
  Strongroom, Clash, Masterwork replaced the earlier placeholders).
- **Volume 4 split pairs** (Glide, Strongroom) — borrowed from the Spectrum BASIC tour-then-deepen
